...
export const smtpFormSchema = z.object({
  host: z.string().trim().min(1, "Host requis").max(255),
  port: z.coerce.number().int().min(1, "Port invalide").max(65535),
...
export async function getSmtpConfigView(): Promise<SmtpConfigView> {
  ...
  const fingerprint = passwordFromEnv
    ? "env"
    : passwordFromDb
  ...
  const configured = Boolean(merged.host && merged.fromEmail);
